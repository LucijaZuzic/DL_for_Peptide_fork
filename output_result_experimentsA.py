...
def filter_dict(minlen, maxlen):
    ...
    for model_name in model_list:
        ...
        if not os.path.isdir('results_processed_seq_experiments_0/' + str(minlen) + "_" + str(maxlen) + "/" + model_name):
            os.makedirs('results_processed_seq_experiments_0/' + str(minlen) + "_" + str(maxlen) + "/" + model_name)
        df_new = pd.DataFrame({"preds": predict_filter, "labels": labs_filter})
        df_new.to_csv('results_processed_seq_experiments_0/' + str(minlen) + "_" + str(maxlen) + "/" + model_name + "/" + str(minlen) + "_" + str(maxlen) + "_" + model_name + "_preds.csv")
...
def print_dict(dicti, mini, maxi):
    ...
    if not os.path.isdir('results_processed_seq_experiments_0/' + str(mini) + "_" + str(maxi)):
        os.makedirs('results_processed_seq_experiments_0/' + str(mini) + "_" + str(maxi))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_seq_experiments_0/' + str(mini) + "_" + str(maxi) + "/" + str(mini) + "_" + str(maxi) + ".csv")
