    ...
    for model_name in model_list:
        ...
        for seed_val in seed_list:
            ...
            for fold_num in range(1, 6):
                models_line_dicts[model_name][seed_val][fold_num] = dict()
                lines_dict = dict()
                predict_filter = []
                seqs_filter = []
                labs_filter = []
                ...
                models_line_dicts[model_name][seed_val][fold_num] = lines_dict
                if not os.path.isdir('results_processed_merged_seq/' + str(minlen) + "_" + str(maxlen) + "/" + model_name + "/seed_" + str(seed_val)):
                    os.makedirs('results_processed_merged_seq/' + str(minlen) + "_" + str(maxlen) + "/" + model_name + "/seed_" + str(seed_val))
                df_new = pd.DataFrame({"preds": predict_filter, "labels": labs_filter, "feature": seqs_filter})
                df_new.to_csv('results_processed_merged_seq/' + str(minlen) + "_" + str(maxlen) + "/" + model_name + "/seed_" + str(seed_val) + "/" + str(minlen) + "_" + str(maxlen) + "_" + model_name + "_seed_" + str(seed_val) + "_fold_" + str(fold_num) + "_preds.csv")
...
def print_dict(dicti, mini, maxi):
    ...
    for model_name in model_list:
        for seed_val in seed_list:
            for fold_num in range(1, 6):
                colnames.append('{} (seed {}, fold {})'.format(model_name, seed_val, fold_num))
    dict_csv_data = dict()
    for c in colnames:
        dict_csv_data[c] = []
    for metric in dicti[model_list[0]][seed_list[0]][1]:
        ...
        for model_name in model_list:
            for seed_val in seed_list:
                for fold_num in range(1, 6):
                    dict_csv_data['{} (seed {}, fold {})'.format(model_name, seed_val, fold_num)].append(dicti[model_name][seed_val][fold_num][metric])
    if not os.path.isdir('results_processed_merged_seq/' + str(mini) + "_" + str(maxi)):
        os.makedirs('results_processed_merged_seq/' + str(mini) + "_" + str(maxi))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_merged_seq/' + str(mini) + "_" + str(maxi) + "/" + str(mini) + "_" + str(maxi) + ".csv")
...
def print_dict_model(dicti, model_name, mini, maxi):
    ...
    for metric in dicti[model_list[0]][seed_list[0]][1]:
        ...
        for seed_val in seed_list:
            for fold_num in range(1, 6):
                dict_csv_data['{} (seed {}, fold {})'.format(model_name, seed_val, fold_num)].append(dicti[model_name][seed_val][fold_num][metric])
    if not os.path.isdir('results_processed_merged_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name):
        os.makedirs('results_processed_merged_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name)
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_merged_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + ".csv")
...
def print_dict_model_seed(dicti, model_name, seed_val, mini, maxi):
    ...
    for metric in dicti[model_list[0]][seed_list[0]][1]:
        ...
        for fold_num in range(1, 6):
            dict_csv_data['{} (seed {}, fold {})'.format(model_name, seed_val, fold_num)].append(dicti[model_name][seed_val][fold_num][metric])
    if not os.path.isdir('results_processed_merged_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val)):
        os.makedirs('results_processed_merged_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_merged_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val) + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + "_seed_" + str(seed_val) + ".csv")
