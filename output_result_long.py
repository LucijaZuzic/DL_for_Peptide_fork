...
def return_seq_predictions_labels(use_model):
    dir_name = 'results_seq/mine/long/'
    ...
    for part_ix in range(1, 12):
        for one_file in os.listdir(dir_name + "/" + str(part_ix)):
            if used_file_name_part in one_file:
                one_file_read = pd.read_csv(dir_name + "/" + str(part_ix) + "/" + one_file)
                print(one_file)
...
def filter_dict(minlen, maxlen):
    ...
    for model_name in model_list:
        ...
        if not os.path.isdir('results_processed_seq_long/' + str(minlen) + "_" + str(maxlen) + "/" + model_name):
            os.makedirs('results_processed_seq_long/' + str(minlen) + "_" + str(maxlen) + "/" + model_name)
        df_new = pd.DataFrame({"preds": predict_filter, "labels": labs_filter, "feature": seqs_filter})
        df_new.to_csv('results_processed_seq_long/' + str(minlen) + "_" + str(maxlen) + "/" + model_name + "/" + str(minlen) + "_" + str(maxlen) + "_" + model_name + "_preds.csv")
...
def print_dict(dicti, mini, maxi):
    ...
    if not os.path.isdir('results_processed_seq_long/' + str(mini) + "_" + str(maxi)):
        os.makedirs('results_processed_seq_long/' + str(mini) + "_" + str(maxi))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_seq_long/' + str(mini) + "_" + str(maxi) + "/" + str(mini) + "_" + str(maxi) + ".csv")
