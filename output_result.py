...
def return_seq_predictions_labels(some_seed, test_number, fold_nr, use_model):
    dir_name = 'results_seq/mine/seed_{}/test_{}/fold_{}/'.format(some_seed, test_number, fold_nr)
    used_file_name_part = 'Test_cla_seed_{}_test_{}_fold_{}_reg_{}'.format(some_seed, test_number, fold_nr, use_model)
...
def filter_dict(minlen, maxlen):
    ...
    for model_name in model_list:
        ...
        for seed_val in seed_list:
            ...
            for test_num in range(1, 6):
                models_line_dicts[model_name][seed_val][test_num] = dict()
                for fold_num in range(1, 6):
                    seqs, predict, labs, one_file = return_seq_predictions_labels(seed_val, test_num, fold_num, model_name)
                    lines_dict = dict()
                    predict_filter = []
                    seqs_filter = []
                    labs_filter = []
...
def print_dict(dicti, mini, maxi):
    ...
    for model_name in model_list:
        for seed_val in seed_list:
            for test_num in range(1, 6):
                for fold_num in range(1, 6):
                    colnames.append('{} (seed {}, test {}, fold {})'.format(model_name, seed_val, test_num, fold_num))
    dict_csv_data = dict()
    for c in colnames:
        dict_csv_data[c] = []
    for metric in dicti[model_list[0]][seed_list[0]][1][1]:
        ...
        for model_name in model_list:
            for seed_val in seed_list:
                for test_num in range(1, 6):
                    for fold_num in range(1, 6):
                        dict_csv_data['{} (seed {}, test {}, fold {})'.format(model_name, seed_val, test_num, fold_num)].append(dicti[model_name][seed_val][test_num][fold_num][metric])
    if not os.path.isdir('results_processed_seq/' + str(mini) + "_" + str(maxi)):
        os.makedirs('results_processed_seq/' + str(mini) + "_" + str(maxi))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + str(mini) + "_" + str(maxi) + ".csv")
    ...
    for metric in dicti[model_list[0]][seed_list[0]][1][1]:
        ...
        for seed_val in seed_list:
            for test_num in range(1, 6):
                for fold_num in range(1, 6):
                    dict_csv_data['{} (seed {}, test {}, fold {})'.format(model_name, seed_val, test_num, fold_num)].append(dicti[model_name][seed_val][test_num][fold_num][metric])
    if not os.path.isdir('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name):
        os.makedirs('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name)
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + ".csv")
...
def print_dict_model_seed(dicti, model_name, seed_val, mini, maxi):
    ...
    for metric in dicti[model_list[0]][seed_list[0]][1][1]:
        ...
        for test_num in range(1, 6):
            for fold_num in range(1, 6):
                dict_csv_data['{} (seed {}, test {}, fold {})'.format(model_name, seed_val, test_num, fold_num)].append(dicti[model_name][seed_val][test_num][fold_num][metric])
    if not os.path.isdir('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val)):
        os.makedirs('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val) + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + "_seed_" + str(seed_val) + ".csv")

def print_dict_model_seed_test(dicti, model_name, seed_val, test_num, mini, maxi):
    colnames = ["Metric"]
    for fold_num in range(1, 6):
        colnames.append('{} (seed {}, test {}, fold {})'.format(model_name, seed_val, test_num, fold_num))
    dict_csv_data = dict()
    ...
    for metric in dicti[model_list[0]][seed_list[0]][1][1]:
        ...
        for fold_num in range(1, 6):
            dict_csv_data['{} (seed {}, test {}, fold {})'.format(model_name, seed_val, test_num, fold_num)].append(dicti[model_name][seed_val][test_num][fold_num][metric])
    if not os.path.isdir('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val) + "/test_" + str(test_num)):
        os.makedirs('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val) + "/test_" + str(test_num))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_seq/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/seed_" + str(seed_val) + "/test_" + str(test_num) + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + "_seed_" + str(seed_val) + "_test_" + str(test_num) + ".csv")
        ...
        if all_seeds_test_ok:
            ...
            for model_name in model_list:
                ...
                for seed in seed_list:
                    ...
                    for tn in range(1, 6):
                        print_dict_model_seed_test(dict_use, model_name, seed, tn, a, b)
        else:
            print("not all ok")
