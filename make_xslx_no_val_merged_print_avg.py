...
for model_name in model_list:
    dict_new[model_name] = dict()
    path_csv = 'results_processed_random_forest/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + ".csv"
    file_csv = pd.read_csv(path_csv)
    for c_ix in range(len(file_csv.columns)):
        c = file_csv.columns[c_ix]
        if model_name in c:
...
dict_csv_new = dict()
dict_csv_new["Metric"] = []
for metric_name in dict_new["Random Forest"]:
    dict_csv_new["Metric"].append(metric_name)
for model_name in dict_new:
    dict_csv_new[model_name] = []
for model_name in dict_new:
    for metric_name in dict_new["Random Forest"]:
        dict_csv_new[model_name].append(np.average(dict_new[model_name][metric_name]))
df_new = pd.DataFrame(dict_csv_new)
df_new.to_csv('results_processed_random_forest/a_result.csv')
...
for model_name in model_list:
    dict_new[model_name] = dict()
    path_csv = 'results_processed_random_forest_20/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + ".csv"
    file_csv = pd.read_csv(path_csv)
    for c_ix in range(len(file_csv.columns)):
        c = file_csv.columns[c_ix]
        if model_name in c:
...
for model_name in dict_new:
    for metric_name in dict_new["Random Forest"]:
        dict_csv_new[model_name].append(np.average(dict_new[model_name][metric_name]))
df_new = pd.DataFrame(dict_csv_new)
df_new.to_csv('results_processed_random_forest_20/a_result.csv')