...
def filter_dict(minlen, maxlen):
    ...
    for model_name in model_list:
        ...
        for seed_val in seed_list:
            ...
            if not os.path.isdir('results_processed_20_seq_no_val/' + str(minlen) + "_" + str(maxlen) + "/" + model_name):
                os.makedirs('results_processed_20_seq_no_val/' + str(minlen) + "_" + str(maxlen) + "/" + model_name)
            df_new = pd.DataFrame({"preds": predict_filter, "labels": labs_filter, "feature": seqs_filter})
            df_new.to_csv('results_processed_20_seq_no_val/' + str(minlen) + "_" + str(maxlen) + "/" + model_name + "/" + str(minlen) + "_" + str(maxlen) + "_" + model_name + "_" + str(seed_val) + "_preds.csv")
...
def print_dict(dicti, mini, maxi):
    ...
    if not os.path.isdir('results_processed_20_seq_no_val/' + str(mini) + "_" + str(maxi)):
        os.makedirs('results_processed_20_seq_no_val/' + str(mini) + "_" + str(maxi))
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_20_seq_no_val/' + str(mini) + "_" + str(maxi) + "/" + str(mini) + "_" + str(maxi) + ".csv")
...
def print_dict_model(dicti, model_name, mini, maxi):
    ...
    for metric in dicti[model_list[0]][seed_list[0]]:
        ...
        for seed_val in seed_list:
            dict_csv_data['{} (seed {})'.format(model_name, seed_val)].append(dicti[model_name][seed_val][metric])
    if not os.path.isdir('results_processed_20_seq_no_val/' + str(mini) + "_" + str(maxi) + "/" + model_name):
        os.makedirs('results_processed_20_seq_no_val/' + str(mini) + "_" + str(maxi) + "/" + model_name)
    df_new = pd.DataFrame(dict_csv_data)
    df_new.to_csv('results_processed_20_seq_no_val/' + str(mini) + "_" + str(maxi) + "/" + model_name + "/" + str(mini) + "_" + str(maxi) + "_" + model_name + ".csv")
