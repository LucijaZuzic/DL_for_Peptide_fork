...
def main():
    for part_ix in range(1, 12):

        if args.task_type == 'Classification':
            df_train = pd.read_csv('Sequential_Peptides/mine/all_data.csv')
            df_test = pd.read_csv('Sequential_Peptides/mine/all_test_labels_long' + str(part_ix) + '.csv')
            train_label = torch.Tensor(np.array(df_train["Label"])).long()
            test_label = torch.Tensor(np.array(df_test["Label"])).long().to(device)
        elif args.task_type == 'Regression':
            df_train = pd.read_csv('Sequential_Peptides/mine/all_data.csv')
            df_test = pd.read_csv('Sequential_Peptides/mine/all_actual_AP_long' + str(part_ix) + '.csv')
        ...
        if args.task_type == 'Classification':
            if not os.path.isdir('models/long/' + str(part_ix) + '/'):
                os.makedirs('models/long/' + str(part_ix) + '/')
            torch.save(model.state_dict(),'models/long/' + str(part_ix) + '/cla_reg_{}_lr_{}_bs_{}.pt'.format(args.model,args.lr,args.batch_size))
        elif args.task_type == 'Regression':
            if not os.path.isdir('models/long/' + str(part_ix) + '/'):
                os.makedirs('models/long/' + str(part_ix) + '/')
            torch.save(model.state_dict(),'models/long/' + str(part_ix) + '/reg_reg_{}_lr_{}_bs_{}.pt'.format(args.model,args.lr,args.batch_size))
        ...
        if args.task_type == 'Classification':
            checkpoint = torch.load('models/long/' + str(part_ix) + '/cla_reg_{}_lr_{}_bs_{}.pt'.format(args.model,args.lr,args.batch_size))
        elif args.task_type == 'Regression':
            checkpoint = torch.load('models/long/' + str(part_ix) + '/reg_reg_{}_lr_{}_bs_{}.pt'.format(args.model,args.lr,args.batch_size))
        ...
        if args.task_type == 'Classification':
            ...
            df_test_seq = pd.read_csv('Sequential_Peptides/mine/all_test_labels_long' + str(part_ix) + '.csv')
            ...
            if not os.path.isdir('results_seq/mine/long/' + str(part_ix) + '/'):
                os.makedirs('results_seq/mine/long/' + str(part_ix) + '/')
            
            df_test_save.to_csv('results_seq/mine/long/' + str(part_ix) + '/Test_cla_reg_{}_Acc_{}_lr_{}_bs_{}.csv'.format(args.model,test_acc,args.lr,args.batch_size))
        ...
        if args.task_type == 'Regression':
            ...
            df_test_seq = pd.read_csv('Sequential_Peptides/mine/all_actual_AP_long' + str(part_ix) + '.csv')
            df_test_save['feature'] = df_test_seq['Feature']
            df_test_save['predict'] = predict
            df_test_save['label'] = labels
            error = []
            ...
            df_test_save.to_csv('results_seq/mine/long/' + str(part_ix) + '/Test_reg_reg_{}_MAE_{}_lr_{}_bs_{}.csv'.format(args.model,MAE,args.lr,args.batch_size))
